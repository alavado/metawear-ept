...
ws.on('message', function (msg) {
  if (msg.startsWith('false')) {
    ...
    if (id && !enviado && grabacion.length > 0) {
      console.log('enviando a strapi...', id)
      console.log('total de datos', grabacion.length)
      axios.post(
        'https://compsci.cl/ept/graphql',
        {
          ...
          "query":"mutation Editar($id: ID!, $datos: JSON) {\n  updateRegistroEpt(input: {where: {id: $id}, data: {datos_emg: $datos}}) {\n    registroEpt {\n      id\n    }\n  }\n}\n"
        }
      )
