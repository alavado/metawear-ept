  ...
  if (data) {
    ...
    if (Date.now() > tiempo + emgSample) {
      ...
      emgBuffer.v = [
        ...emgBuffer.v,
        ...pythonBuffer
          .slice(0, pythonBuffer.lastIndexOf('\n'))
          .split('\n')
          .filter(s => s)
      ]
      emgBuffer.t.push(tiempo - startTime)
...
const ws = new WebSocket('ws://compsci.cl:2304/input')
ws.on('open', function open() {
  ws.send(JSON.stringify({ r: rotationsRead, e: emgBuffer, t: Date.now() }))
})
ws.on('message', function (msg) {
  ws.send(JSON.stringify({ r: rotationsRead, e: {v:[emgBuffer.v[0]],t:[emgBuffer.t[0]]}/*emgBuffer*/, t: Date.now() }))
  emgBuffer = { v: [], t: [] }
})
...
async function start(options, config, cache, cacheFile) {
  ...
  for(let d of config['devices']) {
    if (dispositivos === conexionesExitosas) {
      console.log(`Ya me conecté a ${dispositivos} dispositivos`)
      break
    }
  ...
  for(let it of devices) {
    ...
    for(let s of Object.keys(config['sensors'])) {
      if (!(s in SensorConfig)) {
        ...
        state['update-graph'] = (data) => rotationsRead[d.address] = data.map(v => Math.round(v * 10000) / 10000)
