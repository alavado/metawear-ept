...
python.stdout.on('data', data => {
  if (data) {
    ...
    if (Date.now() > tiempo + emgSample) {
      ...
      emgRecord = [...emgRecord, ...emgBuffer.v]
...
ws.on('open', function open() {
  winston.info('Conexión WSS exitosa')
  ws.send(JSON.stringify({ r: rotationsRead, e: emgBuffer, t: Date.now() }))
})
ws.on('message', function (msg) {
  ws.send(JSON.stringify({ r: rotationsRead, e: {v:[emgBuffer.v.reduce((sum, v) => sum + v / emgBuffer.v.length)], t:[emgBuffer.t[0]]}, t: Date.now() }))
  emgBuffer = { v: [], t: [] }
})
ws.on('error', () => {
