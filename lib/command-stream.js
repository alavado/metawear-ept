...
python.stdout.on('data', data => {
  if (data) {
    ...
    if (Date.now() > tiempo + emgSample) {
      ...
      if (buffer.length > 0) {
        emgBuffer.v.push(buffer.reduce((sum, v) => sum + v) / buffer.length)
...
ws.on('open', function open() {
  ws.send(JSON.stringify({r: rotationsRead, e: Number(emgRead), t: Date.now()}))
})
    ...
    for(let s of Object.keys(config['sensors'])) {
      if (!(s in SensorConfig)) {
        ...
        state['update-graph'] = createWindow !== undefined ? 
            ...
            (data) => rotationsRead[d.address] = data.map(v => Math.round(v * 10000) / 10000)
